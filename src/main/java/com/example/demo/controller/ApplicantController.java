...
import com.example.demo.service.imp.ApplicantServiceImp;
...
public class ApplicantController {
    ...
    private final ApplicantServiceImp applicantServiceImp;

    @GetMapping("/export/csv")
    public ResponseEntity<Resource> getContactsCsv() {
        String filename = "contacts.csv";
        InputStreamResource file = applicantServiceImp.getLeadCsv();
    ...
    public ResponseEntity<HttpStatus> importCsv(@RequestParam("file") MultipartFile file) throws IOException {
        applicantServiceImp.importCsv(file, null);
    ...
    public ResponseEntity<Map<String, Object>> findAll(
        ...
        try {
            final Map<String, Object> response = applicantServiceImp.findAll(page, size);
    ...
    public ResponseEntity<Map<String, Object>> queryFilter(
            ...
            @RequestParam(defaultValue = "10") int size){
        try {
            final Map<String, Object> response = applicantServiceImp.queryFilter(filterType, name, surname, page, size);
            return new ResponseEntity<>(response, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
        }
